---
description: 设计系统（下）— 间距系统（p-3硬约束）、圆角/阴影/过渡、样式覆盖规则、设计Token使用原则、Tailwind常用类名速查。
paths:
  - "**/*.vue"
  - "**/*.css"
---
...
- ✅ 颜色/字体/间距/圆角/阴影必须从项目设计系统引用，禁止硬编码
- ✅ EP 主题色（primary/success/warning/danger/info）必须用 CSS 变量引用
- ✅ 间距必须用 Tailwind 原子类，禁止硬编码 px 值
- ❌ 禁止使用 Tailwind 默认调色板（`bg-gray-*` / `text-blue-*` / `bg-red-500` 等）
- ✅ `bg-white` 和 `bg-transparent` 是例外，允许使用

> 具体的色值、间距档位、字体规格由项目 `project_config/design_system.md` 定义。
